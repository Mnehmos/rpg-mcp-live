...
Runtime mundane items also use `content_compile`, but their immutable runtime definition is bridged into one ordinary actor-owned inventory instance. The inventory instance keeps the canonical runtime instance ID, authored material/tag description, owner, container, quantity, and normal inventory provenance; existing inventory transfer, capacity, equip, use, and drop validation remain authoritative. A derived item must provide a new stable key plus existing source definition/instance IDs, a recipe key, and a plain-language modification. The compiler records that provenance and creates a new definition/instance without mutating the source definition or its other instances. Unsupported mechanical fields are rejected, and stable content/instance IDs plus the existing command idempotency record prevent duplicate replay. Runtime-item inventory links are normalized across reloads; runtime content remains the canonical definition while inventory remains authoritative for ownership and location. Executable item effects, economy/fencing, and partial stack splitting remain deferred.

Runtime arcane synthesis also enters through `content_compile`, but it is executable only after the engine applies the reviewed `runtime-arcane-synthesis-v1` policy. The proposal may name one exact installed `open5e:spell:` primitive and the literal `damage-only` modification; it may not author level, range, target count, duration, concentration, attack/save, damage, healing, or resource mechanics. The engine copies the primitive's reviewed single-target, non-concentration spell-attack damage effect, derives its level and other casting values, and persists a `runtime:spell:` definition at execution tier 2 with the primitive key and policy revision as provenance. The first slice admits only primitives at level 1 or lower, range 120 feet or less, and average base damage 20 or less; unsupported or over-budget requests reject without mutation. Learned, prepared, and cast runtime spells use the ordinary spell kernel and survive normalization/restart. New effect primitives, arbitrary scripting, and non-damage synthesis remain out of scope.
