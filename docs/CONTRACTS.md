...
Character death creates one persisted remains record and atomically transfers ordinary inventory to its stable ID. A matching world-object instance remains scene-owned and topologically valid, but its shared item ID associates it with the remains and blocks direct interaction until `remains_action` recovers it. That closed command loots one existing item, derives the sole reviewed `dragonborn-scale-v1` harvest, or explicitly cleans up decayed remains; callers cannot supply yield, eligibility, decay, or cleanup outcomes. The `ordinary-remains-v1` decay threshold snapshots reviewed weather at death and advances only through authoritative campaign time. Cleanup retains the ledger and provenance, and it rejects while a linked critical world object still requires recovery. See ADR-H36.
...
The reference engine stores character currency as validated `gold`, `silver`, and `copper` denominations. The web adapter projects those denominations into the host's compatibility `character.currency.copper` value and derives the display breakdown using 100 cp = 1 gp and 10 cp = 1 sp. `character.gold` is a compatibility projection only. Carrying capacity is derived from Strength at 15 pounds per point. DM-authored encounter loot supplies its own typed items, XP, and copper; the engine does not inject fixed demo rewards.
