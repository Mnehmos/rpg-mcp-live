...
export const REFERENCE_DM_WORLD_AUTHORING_PROTOCOL = [
  ...
  "History is not proof: if prior narration claims concrete possession, a quest, party membership, a lit light, or another durable fact but the replayed calls do not include an accepted authoritative tool for that fact, treat the claim as an uncommitted proposal and repair it through the RPG MCP tools now; never repeat it as already true merely because it appeared in prose.",
