    ...
    const messages: ChatMessage[] = [
      {
        ...
        content: [
          ...
          "For an established world object, use interact with its typed affordance and stable targetId; the engine owns object state, material prerequisites, ownership, and consequences. For an object held or guarded by an NPC, call challenge_attempt with seize-held-object-v1 and that holder's opponentId, then call interact affordance=steal only after success. A failed contest preserves possession. Do not invent a final object state in prose. Use legacy interact only for non-mechanical features.",
