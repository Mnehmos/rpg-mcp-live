...
const replayFixtures: readonly ReplayFixture[] = [
  ...
  { kind: "cast_spell", tool: "cast_spell", build: () => { const state = wizardCombatState(); return { state, command: parseCommand({ kind: "cast_spell", spellKey: BURNING_HANDS, targetIds: [], area: { geometryRevision: state.combat.tactical.geometry.revision, aim: { ...state.combat.tactical.actorPosition, x: state.combat.tactical.actorPosition.x + 1 } } }) }; } },
